...
import ClientSideDashboardPage from './ClientSideDashboardPage';
...
export default async function DashboardPage({ params }: { params: { id: string } }) {
  ...
  if (!config && isUUID) {
    try {
      ...
      console.error('DashboardPage: Error getting product config:', error);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-background">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground mb-4">Dashboard no encontrado</h1>
  ...
  return (
    <DirectChatbotAccess chatbotId={id}>
      <ClientSideDashboardPage config={config} />
    </DirectChatbotAccess>
  );
}
